...
def main() -> None:
    ...
    collator = MTPCollator(tokenizer, pad_to_length=config.pad_to_length)
    loader = DataLoader(
        train_dataset,
    ...
    for step in pbar:
        ...
        for micro_step in range(config.gradient_accumulation_steps):
            ...
            for key, value in logs.items():
                accum_logs[key] = accum_logs.get(key, 0.0) + float(value)
        ...
        logs = {key: value / config.gradient_accumulation_steps for key, value in accum_logs.items()}
